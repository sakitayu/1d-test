...
「プロダクション想定」を 12-Factor 系の `fail-fast on missing env` で読むのは internal admin tool の文脈の話。public-facing で GitHub の公開データを扱うクライアント（Octokit / GitHub REST API を直接叩く運用 / github.com の web UI 等）は token を opt-in 扱いにしており、未認証でも graceful に動くのが慣習。本アプリも同じパターンを採用。レビュアーがクローン直後に `pnpm dev` で即起動できることが、production らしさの一部だと判断した。
...
セマンティック HTML（`<button>` / `<a href>` / `<form>` / `<label>` / `<search>` / `<time>` を div で代替しない）、heading 階層（home に sr-only `<h1>`、RepoCard に `<h2>`、詳細ページに visible `<h1>`）、`focus-visible:ring-*` の明示、`<label>` と `<input>` の `htmlFor`/`id` 紐付け、装飾アイコンの `aria-hidden`、avatar の `alt={owner.login}`、skip-to-content リンク、ページネーションの `aria-disabled`、ローディングの `aria-busy` / `aria-live`、検索結果件数と 0 件状態の `role="status"`、エラーバウンダリの `role="alert"`、外部リンクへの `(新しいタブで開く)` sr-only 補強。public-facing なアプリでは後付けより初期から織り込む方が結果的にコストが低い、という前提で実装した。

## 工夫点・拘ったポイント

本実装で最も拘ったのは、**「production-faithful」をスローガンではなく具体的な技術判断の積み重ねとして実現すること** だった。PAT を必須にせず未認証クォータでも graceful に動かす設計、Server Component から GitHub API を直接叩いて token をクライアントバンドルに一切漏らさない構造、`lib/github.ts` を Next.js の制御フロー (`notFound()` / `redirect()`) から切り離してドメイン例外を throw するレイヤリング、独自プロパティを持つエラーをシリアライズ境界を越えさせずに page.tsx 内で完結させる判断 — これらはいずれも公開 GitHub クライアントの慣習に倣ったもので、評価者がクローン直後に追加設定なしでアプリを動かせる「production らしさ」を技術的に裏付ける選択になっている。フッターも「Built with Next.js」のような技術スタックの誇示を排除し、データ提供元の attribution のみに整理した。

GitHub API の細部にも踏み込んでいる。`watchers_count` は歴史的経緯で `stargazers_count` と同値を返すため、詳細ページの「Watcher 数」には検索 API では取れない `subscribers_count`（通知購読者数）を採用した。レート制限は Search API（30 req/分）と Core REST API（5,000 req/時）が独立した枠で管理されるため `X-RateLimit-Resource` ヘッダで判別し、リセット時刻を `HH:MM` と「あと N 分」の両形式で UI に提示して復旧見込みを伝える。検索クエリの 256 文字制限は `parseSearchParams` 純粋関数で検証し、超過時は API を呼ばずに UI で即フィードバックすることで API クォータを構造的に節約している。`parseSearchParams` の戻り値を `'empty' | 'valid' | 'invalid'` の discriminated union にすることで、`page.tsx` の switch が型レベルで網羅される構造になっている。

UX とアクセシビリティは **後付けではなく初期設計の一部** として組み込んだ。Tailwind v4 のデフォルトより少し長い 200ms ease-out を全 hover / focus interaction に統一して機械的な切り替えを排し、ローディング中のスケルトンは実コンテンツと高さを揃えて Cumulative Layout Shift をゼロに近づけ、URL から `q` を取って input にクエリを保持することで遷移時の違和感を消した。ネイティブ × ボタンの dark モード可読性（Primer の x-icon を mask-image でカスタム描画）、詳細から戻る際のスクロール位置復元（`router.back()` + 3 段階 fallback）、外部リンクの「新しいタブで開く」sr-only 通知、heading 階層 / `role="status"` / `role="alert"` / focus ring の全要素一括 transition といった粒度で配慮しており、一つひとつは小さいが合計が「気を配って作った」という体感に直結する箇所として丁寧に詰めた。色は GitHub Primer のセマンティックトークン（accent.fg `#0969da`、dark モードの header `<` body `<` card という 3 段階サーフェス階層）に揃え、ファビコンも Primer の mark-github を採用してタブ・ブックマークまでブランドが一貫するようにしている。
...
- **アクセシビリティ**: skip link、`<search>` / `<time>` などのセマンティック要素、heading 階層を home の sr-only `<h1>` から RepoCard `<h2>` まで連続させる、`focus-visible` ring、検索結果件数と 0 件状態に `role="status"`、エラーバウンダリに `role="alert"`、ローディングの `aria-busy` / `aria-live`、ページネーションの `aria-disabled`、avatar の意味のある alt、外部リンクには `(新しいタブで開く)` の sr-only 補強テキスト
...
- **ホバー時 transition を 200ms ease-out に統一**: Tailwind v4 デフォルト 150ms linear は機械的に感じられるため、`globals.css` の `--default-transition-duration` / `--default-transition-timing-function` を global default で上書きし、全 hover / focus interaction を 200ms ease-out (UI 業界の 150-300ms 帯の中央付近、Material Design の "short4" に該当) に揃えて「ふわっと」した質感を出した。Tailwind の `transition-colors` は box-shadow を含まない (= focus ring が瞬間表示になる) ため全要素を `transition` 一括に変更し、hover と focus が同じリズムで動くよう統一。`group-hover:` で子要素プロパティを変える箇所では、CSS の transition が親→子に継承されない仕様を踏まえて子側にも `transition` を付与している
- **ロード中スケルトンの高さを実コンテンツと厳密一致 + クエリ保持**: `loading.tsx` のスケルトンに、検索結果ヘッダー (`「〜」の検索結果: 約 N 件`) と同じ高さ (`text-sm` = `h-5`) のプレースホルダを `SearchForm` と `SkeletonCard` 群の間に挟むことで、ロード→結果到着時に RepoCard が縦方向にジャンプしない (CLS 抑制)。加えて `loading.tsx` を Client Component 化し `useSearchParams` で URL から `q` を取って SearchForm に渡すことで、検索送信中も入力欄に現在のクエリが保持され、placeholder が一瞬チラ見えしない
- **検索 input のネイティブ × ボタンを theme 適応でカスタム描画**: WebKit デフォルトの `::-webkit-search-cancel-button` は固定の暗めグレーで dark モード背景でコントラストが不足するため、Primer の x-16 octicon を `mask-image` として焼き `background-color` で着色する方式に置き換え、light/dark どちらでも 4.5:1+ を確保。`:placeholder-shown` 擬似クラスで `opacity: 0/1` + `pointer-events` を制御し、入力中の出現も 200ms ease-out でフェード
- **production-faithful なフッター**: 「Built with Next.js」のような技術スタック誇示は production サービスの慣習に反するため排除し、「データ提供: GitHub」のみのシンプルな data attribution に整理。リンク先も API ドキュメント (開発者向け) から github.com (エンドユーザーが期待する遷移先) に変更
- **詳細から戻る時にスクロール位置・クエリ・ページを完全復元**: 詳細ページの「戻る」リンクを `BackLink` (Client Component) として切り出し、3 段階で fallback。JS 無効時は `<a href="/">` で検索トップへ遷移、JS 有効時はブラウザ履歴がある場合 `router.back()` で実ナビゲーションを巻き戻し検索結果のスクロール位置までピクセル単位で復元、履歴が無い (深いリンク・新規タブ) 場合は href の `/` にフォールバック。リンクの underline 出現も瞬間表示ではなく `text-decoration-color: transparent → currentColor` の transition で 200ms フェードさせ、他の interaction と質感を揃えた
